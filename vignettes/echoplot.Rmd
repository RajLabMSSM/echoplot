...
The plot is centered on the lead/index SNP. If a list is supplied to plot_zoom
...
* Save a high-resolution versions the plot by setting `save_plot=T`.  
  ...
  *multiview_<locus>_<plot_zoom>.jpg*.
...
* You can easily generate the same locus plot at multiple zoomed in views by supplying a list to `plot_zoom`.  
...
```{r extract view}
view1_list <- trk_plot_list[[plot_zoom]]
